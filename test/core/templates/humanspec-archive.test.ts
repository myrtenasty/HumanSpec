  ...
  it('contains context, verification, forced-archive, and canonical-operation gates', () => {
    for (const [label, body] of bodies) {
      ...
      for (const marker of [
        ...
        'template versions',
        'parse issues',
        'latest bounded version-1 learning-feedback result',
        'learning-status: complete',
        'overall verification disposition and its `learning-status` are separate',
  ...
  it('contains preview confirmation, pending reconciliation, and ownership boundaries', () => {
    for (const [label, body] of bodies) {
      ...
      for (const marker of [
        ...
        '--adaptive',
        'completed-slice removal',
        '--reject-candidate',
        'candidate disposition',
        'never create its change directory',
        'pending state',
        'Retry reconciliation',
        'must never re-infer an outcome from narrative text',
        'rerun specification synchronization',
  ...
  it('covers blocking and forced journeys without unauthorized writes', () => {
    for (const [label, body] of bodies) {
      ...
      for (const marker of [
        ...
        'only a complete learning status may replay mastered records',
        'supported gaps and review items remain useful',
        'contract reference, observed evidence, consequence, and bounded learner next step',
